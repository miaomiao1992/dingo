Please provide a comprehensive analysis of the optional syntax tree parser layer for Dingo. Your response should be a complete markdown document that includes:

1. **Executive Summary**: Clear YES/NO recommendation with brief justification (2-3 sentences)
2. **Technology Choice**: If YES, which parser technology (Participle, Tree-sitter, custom, PEG) and why
3. **Architecture Design**: How to integrate (plugin-invoked, feature-gated, lazy) with code examples
4. **Cost-Benefit Analysis**: Concrete numbers for implementation effort, performance impact, features enabled
5. **Implementation Roadmap**: Timeline and phases
6. **Risk Assessment**: Key risks and mitigation strategies
7. **Precedents**: Examples from other transpilers that support your recommendation

Be thorough and detailed - this is a critical architecture decision.
